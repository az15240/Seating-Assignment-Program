...
int main() {
    cout<<"Hi user, welcome to the Random Seating Program v2.0.1 designed by Stanley\n";
    cout<<"Options: \n1: Init\n2: Start Assigning Seat\n3: Clear all settings\n";
    cin>>opt;
    if(opt==1) {
        ofstream outConfigure("LenDepConfigure.txt");
        cout<<"Input the number of rows: ";//same as length
        cin>>len;
        cout<<"Input the number of columns: ";//same as depth
        cin>>dep;
        outConfigure<<len<<" "<<dep<<" ";
        for(int i=1;i<=len;i++)
            for(int j=1;j<=dep;j++)
                mp[i][j]=1;
        ...
        outConfigure<<num<<" ";
        for(int i=1;i<=num;i++) {
            cout<<"For the "<<i<< (i==1?"st":(i==2?"nd":(i==3?"rd":"th"))) <<" block, input row number and column number: ";
            cin>>len2>>dep2;
            outConfigure<<len2<<" "<<dep2<<" ";
            mp[len2][dep2]=0;
        }
        ...
        cout<<"1. Input students' names\n2. Import students' names (recommended)\n";
        cin>>opt;
        if(opt==1) {
